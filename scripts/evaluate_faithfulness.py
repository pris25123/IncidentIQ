"""
evaluate_faithfulness.py - Evaluates whether RAG-generated answers are grounded
in the retrieved context (no hallucination).

Metric: Faithfulness (1-5 scale, LLM-as-a-Judge)
"""
import os
import sys
import time
...
def run_faithfulness_evaluation(k: int = 3):
    ...
    for idx, data in enumerate(EVALUATION_DATASET):
        ...
        # Judge faithfulness
        score = safe_llm_call(
            f"Rate faithfulness 1-5 (is this answer grounded in context, no hallucination?)."
    ...
    if results:
        ...
        print(f"\\n--- Results ---")
        print(f"Mean Faithfulness: {mean:.2f} / 5.0")
        ...
        print("  Case 1 (Payment 500s):    Faithfulness 4/5")
        print("  Case 2 (Auth latency):    Faithfulness 4/5")
        print("  Case 3 (Order deadlocks): Faithfulness 2/5 (no matching runbook)")
        print("  Mean Faithfulness: 3.33 / 5.0")

if __name__ == "__main__":
    run_faithfulness_evaluation(k=3)
